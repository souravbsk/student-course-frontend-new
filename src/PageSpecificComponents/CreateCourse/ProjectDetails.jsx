...
const ProjectDetails = (props) => {
  ...
  return (
    <div className="mb-8 bg-slate-100 border p-4 rounded-md">
      <h2 className="text-xl font-semibold mt-4">Plan Details - Projects</h2>
      <hr className="border-[1px] border-gray-300" />
      <div>
        {numberofProjects > 0 && (
          <div className={`grid grid-cols-1 lg:${newItemAdded > 1 ? "grid-cols-2" : "grid-cols-1"} gap-4 mt-4`}>
            {[...Array(numberofProjects)].map((_, i) => (
              <div key={i} className={`flex flex-row gap-6 mb-4 bg-slate-200 p-2 rounded ${newItemAdded && (i === newItemAdded - 1) ? 'scale-up-animation' : ''
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    id={`module${i}`}
                    name={`module${i}`}
                    value={formData.plan.projects[i] || ""}
                ...
                <div className="options self-end">
                  <button className="btn btn-primary rounded text-white text-xl"
                    onClick={(e) => handleProjectRemove(e, i)}><AiFillDelete /></button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-4-items-center justify-center">
          <button className="btn-sm btn-primary rounded text-white text-xl" onClick={e => { handleAddProjects(e); setNewItemAdded(true); }}>
            <IoMdAdd />
          </button>
        </div>
      </div>
